...
function FitRoute({ route, origin, destination, stops = [] }) {
  const map = useMap();
  ...
  }, [destination, map, origin, route, stops]);
...
function stopIcon(index) {
  return L.divIcon({
    ...
    popupAnchor: [0, -14],
    html: `
      <div class="grid size-8 place-items-center rounded-full bg-amber-400 text-slate-950 shadow-lg ring-2 ring-white">
        <strong class="text-sm font-black">${index + 1}</strong>
      </div>
    `
  });
}
...
function getZoneCenter(zone) {
  if (zone.center?.lat && zone.center?.lng) {
    return [Number(zone.center.lat), Number(zone.center.lng)];
  }
  ...
  return (
    <div className="absolute inset-0 z-0">
      <MapContainer center={center} zoom={12} scrollWheelZoom className="h-full w-full" zoomControl={false} attributionControl={false}>
        ...
        <FitRoute route={route} origin={origin} destination={destination} stops={stops} />
        ...
        {stops.map((stop, index) => (
          <Marker key={stop.id || `${stop.lat}-${stop.lng}-${index}`} position={[Number(stop.lat), Number(stop.lng)]} icon={stopIcon(index)}>
            <Popup>{stop.name || `Parada ${index + 1}`}</Popup>
          </Marker>
        ))}
        {destination && (
