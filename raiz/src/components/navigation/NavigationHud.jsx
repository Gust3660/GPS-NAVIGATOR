...
export default function NavigationHud({
  ...
  return (
    <div className="pointer-events-none fixed inset-0 z-40">
      <div className="absolute left-3 top-3 w-[min(520px,calc(100vw-1.5rem))] overflow-hidden rounded-3xl bg-orange-700 text-white shadow-2xl shadow-red-950/20 ring-1 ring-orange-200/25 dark:bg-red-900">
        ...
        <div className="flex w-fit items-center gap-3 rounded-tr-3xl bg-red-900/65 px-6 py-4 text-white dark:bg-red-950">
      ...
      <div className="absolute right-4 top-4 grid gap-3">
        <div className="grid size-20 place-items-center rounded-full border-4 border-red-600 bg-white text-slate-950 shadow-xl dark:border-orange-300 dark:bg-slate-950 dark:text-orange-300">
          <strong className="text-3xl leading-none">{Math.round(speedKmh || 0)}</strong>
          <span className="-mt-2 text-xs font-bold text-slate-950 dark:text-orange-200">km/h</span>
        </div>
        <button
          type="button"
          className={cx(
            'pointer-events-auto grid size-16 place-items-center rounded-full border border-white/70 bg-white/45 p-1 text-slate-950 shadow-xl shadow-slate-900/20 ring-1 ring-white/70 backdrop-blur-2xl transition hover:bg-white/65 active:bg-orange-500/80 dark:border-white/10 dark:bg-slate-950/40 dark:text-white dark:ring-white/15 dark:hover:bg-slate-900/65',
            compassActive && 'ring-2 ring-orange-400 dark:ring-orange-300',
        ...
        <button
          type="button"
          className="pointer-events-auto grid size-16 place-items-center rounded-full border border-white/70 bg-white/45 text-slate-950 shadow-xl shadow-slate-900/20 ring-1 ring-white/70 backdrop-blur-2xl transition hover:bg-white/65 active:bg-orange-500/80 active:text-white dark:border-white/10 dark:bg-slate-950/40 dark:text-white dark:ring-white/15 dark:hover:bg-slate-900/65 dark:active:bg-orange-400/90 dark:active:text-slate-950"
        ...
        <div className="flex h-36 w-16 flex-col items-center justify-between rounded-full border border-white/70 bg-white/45 py-4 text-slate-800 shadow-[0_18px_46px_rgba(15,23,42,0.25)] ring-1 ring-white/70 backdrop-blur-2xl dark:border-white/10 dark:bg-slate-950/40 dark:text-white dark:ring-white/15 max-[680px]:h-40 max-[680px]:w-[4.5rem]">
          <button type="button" className="pointer-events-auto grid size-11 place-items-center rounded-full text-4xl font-light leading-none transition hover:bg-white/65 active:bg-orange-500/80 active:text-white dark:hover:bg-slate-900/65 dark:active:bg-orange-400/90 dark:active:text-slate-950 max-[680px]:size-12" aria-label="Acercar mapa" title="Acercar mapa" onClick={() => onZoom?.('in')}>+</button>
      ...
      <button
        type="button"
        className="pointer-events-auto absolute bottom-28 left-7 inline-flex min-h-14 items-center gap-3 rounded-full border border-white/70 bg-white/45 px-6 text-lg font-black text-orange-700 shadow-2xl shadow-slate-900/15 ring-1 ring-white/70 backdrop-blur-2xl transition hover:bg-white/65 active:bg-orange-500/80 active:text-white dark:border-white/10 dark:bg-slate-950/40 dark:text-orange-300 dark:ring-white/15 dark:hover:bg-slate-900/65 dark:active:bg-orange-400/90 dark:active:text-slate-950 max-[680px]:bottom-44"
        onClick={onRecenter}
      ...
      <div className="absolute bottom-0 left-0 right-0 overflow-hidden rounded-t-[28px] bg-white text-slate-950 shadow-2xl ring-1 ring-slate-200 dark:bg-slate-950 dark:text-white dark:ring-slate-800">
        ...
        <div className="border-b border-slate-200 px-6 py-3 dark:border-slate-800">
          ...
          <div className="h-2 overflow-hidden rounded-full bg-slate-200 dark:bg-slate-800">
            <div className="h-full rounded-full bg-orange-500 transition-all duration-500 dark:bg-orange-400" style={{ width: `${progressValue}%` }} />
          </div>
        </div>
        <div className="flex items-center">
          <button type="button" className="pointer-events-auto m-5 grid size-16 shrink-0 place-items-center rounded-full border border-slate-300 text-slate-700 transition hover:bg-slate-100 active:bg-orange-500 active:text-white dark:border-slate-700 dark:text-slate-100 dark:hover:bg-slate-800 dark:active:bg-orange-400 dark:active:text-slate-950" onClick={onStop} aria-label="Detener navegacion">
            <X size={24} />
          </button>
          ...
          <button
            type="button"
            className="pointer-events-auto mx-5 grid size-12 shrink-0 place-items-center rounded-full text-slate-600 transition hover:bg-slate-100 active:bg-orange-500 active:text-white dark:text-slate-200 dark:hover:bg-slate-800 dark:active:bg-orange-400 dark:active:text-slate-950"
...
function CompassIcon({ heading = 0, active = false }) {
  return (
    ...
    >
      ...
      {active && <span className="absolute inset-0 rounded-full ring-2 ring-orange-400/70 dark:ring-orange-300/70" />}
    </span>
  );
}
...
function Metric({ value, label }) {
  return (
    <div className="py-5">
      <strong className="block text-3xl font-black leading-tight text-orange-600 dark:text-orange-300">{value}</strong>
