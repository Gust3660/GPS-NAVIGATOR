...
def osrm_excluded_classes(avoid_tolls=False, avoid_highways=False):
    excluded = []
    if avoid_tolls:
        excluded.append("toll")
    if avoid_highways:
        excluded.append("motorway")
    return excluded
...
def fetch_osrm_routes(points, alternatives=True, timeout=10, excludes=None):
    ...
    if excludes:
        query["exclude"] = ",".join(dict.fromkeys(excludes))
    ...
    for route in data["routes"]:
        ...
        for leg in route.get("legs", []):
            for step in leg.get("steps", []):
                ...
                name = step.get("name") or "vialidad sin nombre"
                ...
                if not instruction:
                    ...
                    if step_type == "depart":
                        ...
                        instruction = f"Gira a la izquierda en {name}"
                    elif modifier == "right":
                        instruction = f"Gira a la derecha en {name}"
                    elif modifier == "straight":
                        instruction = f"Continua por {name}"

                steps.append({
                    "instruction": instruction,
                    "street": name,
                    "distance_meters": round(float(step.get("distance") or 0), 1),
...
def nearby_red_zones_for_segment(start, end, red_zones, padding=0.12):
    ...
    min_lat = min(start[0], end[0]) - padding
    max_lat = max(start[0], end[0]) + padding
    min_lng = min(start[1], end[1]) - padding
    max_lng = max(start[1], end[1]) + padding

    nearby = []
    for zone in red_zones:
        zone_min_lat, zone_max_lat, zone_min_lng, zone_max_lng = zone_bbox(zone)
        if zone_max_lat < min_lat or zone_min_lat > max_lat:
            continue
        if zone_max_lng < min_lng or zone_min_lng > max_lng:
            continue
        nearby.append(zone)
    return nearby
...
def route_grid_segment(start, end, red_zones=None, step=0.003):
    ...
    if distance > 100_000:
        step = max(step, 0.02)
    elif distance > 35_000:
        step = max(step, 0.01)
    elif distance > 12_000:
        step = max(step, 0.006)
    ...
    if max_lat_index * max_lng_index > 45_000:
        return road_like_dogleg_segment(start, end, red_zones)
    frontier = []
...
def road_route_candidates(origin, destination, red_zones, stops=None, avoid_tolls=False, avoid_highways=False):
    ...
    exempt_points = required_points
    candidates = []
    excludes = osrm_excluded_classes(avoid_tolls=avoid_tolls, avoid_highways=avoid_highways)

    try:
        for route in fetch_osrm_routes(required_points, alternatives=len(required_points) == 2, excludes=excludes):
        ...
        mode = f" con exclusiones {', '.join(excludes)}" if excludes else ""
        raise RuntimeError(f"No se pudo consultar OSRM{mode}: {exc}") from exc
    ...
    for zone in hit_zones:
        for waypoint in candidate_waypoints_for_zone(zone, padding=0.012):
            if point_inside_red_zone(waypoint[0], waypoint[1], red_zones):
                continue
            try:
                for route in fetch_osrm_routes([origin, waypoint, destination], alternatives=False, excludes=excludes):
                    score, hits = score_road_route(route, red_zones, exempt_points=exempt_points)
                    candidates.append({
                        **route,
                        "score": score,
...
def calculate_road_network_route(origin, destination, red_zones, stops=None, avoid_tolls=False, avoid_highways=False, traffic_context=None):
    stops = stops or []
    required_points = [origin, *stops, destination]
    exempt_points = required_points
    excluded_classes = osrm_excluded_classes(avoid_tolls=avoid_tolls, avoid_highways=avoid_highways)
    candidates = road_route_candidates(
        origin,
        destination,
        red_zones,
        stops=stops,
    ...
    direct_line = build_route_coords(required_points, red_zones=[])
    avoided_zones = [
        zone["name"]
        for zone in red_zones
...
def calculate_local_route(origin, destination, red_zones, avoid_tolls=False, avoid_highways=False, traffic_context=None):
    if avoid_tolls or avoid_highways:
        restrictions = []
        if avoid_tolls:
            restrictions.append("casetas de cobro")
        if avoid_highways:
            restrictions.append("autopistas")
        return {
            "error": (
                "El motor local no puede garantizar que se eviten "
                + " y ".join(restrictions)
                + "; se requiere OSRM con exclusiones o INEGI Sakbe libre."
            )
        }
